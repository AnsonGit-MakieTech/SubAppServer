...
t1 = timeit.timeit("foo()", setup=setup, number=1000000)

# Function call via exec
t2 = timeit.timeit("exec('foo()')", setup=setup, number=100000)

# Function call via precompiled exec
code = compile("foo()", "<string>", "exec")
t3 = timeit.timeit("exec(code)", setup=setup + "\ncode=compile('foo()', '<string>', 'exec')", number=1000000)

print("Normal call:", t1)
print("Exec string:", t2)
print("Exec precompiled:", t3)
